...
public:
  ...
  bool write_single(int joint, double speed, const ros::Duration &period)
  {
    ...
    if (diff > 1.0)
    {
      // std::cout << "new speed " << joint << ": " << speed_mapped << std::endl;
  ...
  void read_single(int joint, const ros::Duration &period)
  {
    ...
    double distance_rad;
    if (bidirectional)
    { // if encoder is counting bidirectional, then it
...
private:
  ...
  std::vector<ros::Subscriber> speed_servers;
  ...
  void SpeedCB(const std_msgs::Float32::ConstPtr& msg, int i)
  {
    cmd[i] = msg->data;
  }
  void timercb(const ros::TimerEvent& ev) {
    auto diff = ev.current_real -ev.last_real;
    this->read (diff);
    this->write(diff );
  }
    ...
    : private_nh("~"), running_(true)
{
  ...
  if (enablePID)
  {
    ...
    {
      auto pid = std::make_shared<control_toolbox::Pid>(1,0,0); // TODO: set gains
      pid->setGains(gains);
  ...
  for (size_t i = 0; i < NUM_JOINTS; i++)
  {
    ...
    speed_servers.push_back(nh.subscribe<std_msgs::Float32>((boost::format("mirte/set_speed_pid_%s") % this->joints[i]).str(),1,  boost::bind(&MyRobotHWInterface::SpeedCB, this, _1, i)));
  }
  ...
  this->timer = nh.createTimer(ros::Duration(1.0/rate),  &MyRobotHWInterface::timercb, this);
