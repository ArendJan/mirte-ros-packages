...
# Imported FrontendLaunchDescriptionSource to handle the .yaml launch file
...
def generate_launch_description():
    ld = LaunchDescription(
        [
            ...
            # Added launch arguments for the MoCap server so they can be overridden from the CLI if needed
            DeclareLaunchArgument(
                "mocap_server",
                default_value="192.168.0.232",
                description="IP address of the VRPN MoCap server",
            ),
            DeclareLaunchArgument(
                "mocap_port",
                default_value="3883",
                description="Port of the VRPN MoCap server",
            ),
        ],
    )
    ...
    bicycle_control = GroupAction(
        actions=[
            ...
            IncludeLaunchDescription(
                PathJoinSubstitution(
                    [FindPackageShare("mirte_dart_control"), "launch", "dart.launch.py"]
                ),
    ...
    ld.add_action(
        GroupAction(
            [
                ...
                rosbridge,
                #usb_cam,
                vrpn_mocap, # Added the vrpn client to the scoped group action
            ],
